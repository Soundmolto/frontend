...
export default class Footer extends Component {
	...
	onPosChange() {
		...
		this.progressBar.setAttribute(
			"style",
			`transform: translateX(${(pos / duration) * 100}%)`
		);
		this.thumb.setAttribute(
			"style",
			`transform: translateX(${(pos / duration) *
				this.thumb.parentElement.clientWidth}px)`
		);
	...
	calculate_amount = currently_playing => {
		...
		return (
			(currently_playing.position / currently_playing.track.duration) *
			100
		);
	};
	...
	onClickPlay() {
		...
		if (this.isCurrentlyPlayingNotEmpty(currently_playing)) {
			const owner = {
				profile: currently_playing.user || currently_playing.owner
			};
	...
	onClickNext() {
		...
		if (this.isCurrentlyPlayingNotEmpty(currently_playing)) {
			...
			if (next) {
				console.log(next);
				const owner = {
					profile: next.user ? next.user : next.owner
				};
	...
	componentDidMount() {
		...
		if (this.props.currently_playing.track != null) {
			...
			if (
				audioPlayer == null &&
				window.document.querySelector("audio") != null
			) {
			...
			window.addEventListener(eventKeys.audioEnded, () => {
				const owner = {
					profile:
						currently_playing.track.owner || currently_playing.owner
				};
				...
				requestAnimationFrame(_ => {
					...
					let owner =
						currently_playing.owner || currently_playing.user;
					if (track != null) {
						if (
							(currently_playing.owner || currently_playing.user)
								.id !== (track.owner || track.user)
						) {
							owner = null;
						}
						playing_now(dispatch, {
		...
		window.document.addEventListener("mouseout", e => {
			...
			if (!from || from.nodeName == "HTML") {
				...
				const e = window.document.querySelector(`.${styles.tooltip}`);
	...
	componentDidUpdate() {
		...
		if (currently_playing.playing === true) {
			if (
				currently_playing.position != null &&
				currently_playing.position <
					this.tracks[currently_playing.track.id]
			) {
				this.tracks[currently_playing.track.id] =
					currently_playing.position;
			}
	...
	componentWillUpdate(nextProps) {
		...
		const currently_playing =
			nextProps.currently_playing || this.props.currently_playing;

		if (
			currently_playing.track &&
			audioPlayer.currentTime > this.tracks[currently_playing.track.id]
		) {
	...
	onMouseMove = e => {
		const { currently_playing, dispatch } = this.props;
		const duration =
			(currently_playing.track && currently_playing.track.duration) || 0;
		...
		if (e.target === this.desktopTrackbar) {
			...
			tooltip.setAttribute(
				"style",
				`transform: translateX(${positionInPage - offset}px)`
			);
		}
	};
	...
	onMouseOut = () => {
		const func = () => {
			...
			this.tooltip.removeEventListener("transitionend", func);
		};
		this.tooltip.classList.remove(styles.show);
		this.tooltip.addEventListener("transitionend", func);
	};
	...
	render({ currently_playing, audioPlayer }) {
		...
		const owner =
			(currently_playing.owner || currently_playing.user) &&
			currently_playing.track &&
			currently_playing.track.user &&
			(currently_playing.track.user.displayName ||
				currently_playing.track.user.url);
		const trackName =
			(currently_playing &&
				currently_playing.track &&
				...
				<div class={styles.notMobile}>
					...
					>
						<div class={styles.start}>
							<div class={styles.artwork}>
								<div
									...
									style={{
										"background-image": `url(${this.getArtwork(
											currently_playing.track || {}
										)})`
									}}
								/>
						...
						<div class={styles.end}>
							<div
								class={styles.volumePanel}
								ref={this.volumePanelRef}
							>
								<Slider
									step={2}
									...
									onInput={e => {
										...
										this.volumeIcon.props.children = [
											this.getVolumeType(this.volume)
										];
							...
							<Button
								...
								onClick={e => {
									...
									this.volumePanel.classList.toggle(
										styles.show
									);
									e.currentTarget.blur();
								}}
							...
							<Button
								ripple
								className={`${styles.button}`}
								onClick={this.toggleQueuePanel.bind(this)}
							>
								<Icon style={{ margin: 0 }}>queue_music</Icon>
							</Button>
							<Button
								ripple
								className={`${styles.button} ${this.state
									.repeat === true && styles.active}`}
