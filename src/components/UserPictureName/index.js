...
const renderProfile = (
	...
	<div class={`vertical-center ${passedClass}`} style={style}>
		...
		{(showUsername === true || show_location === true) && (
			<h1 class={`${h1_class} ${styles.usernameCustom}`}>
				...
				{show_location && (
					<small class={className}>{user.location || ""}</small>
				)}
			</h1>
		)}
...
export class UserPictureName extends Component {
	render(props) {
		return props.linksToProfile ? (
			<a
				href={`/${props.user.url}`}
				class={`${styles.noTextDecoration} ${props.class || ""}`}
				onClick={props.onClickProfile || function() {}}
